  ...
  let%lwt case_data =
    Lwt_list.filter_map_p
      (fun key ->
        ...
        Lwt.catch
          (fun () ->
            let%lwt data = Lwt.map (Case.process ~name_matcher uri) (Oscn.fetch `GET uri) in
...
let exec_case S.{ person = request; case_uri = uri } =
  ...
  let json : Yojson.Safe.t =
    `Assoc [ "case_data", Oscn.prepare_case ~name_matcher case_data |> Option.value ~default:`Null ]
  in
  Lwt.return json
...
| _ :: "search" :: ln :: rest ->
  ...
  let%lwt () =
    Lwt_io.with_file
      ~flags:Unix.[ O_WRONLY; O_NONBLOCK; O_TRUNC; O_CREAT ]
      ~mode:Output "cases.json"
      (fun oc -> Lwt_io.write oc out)
  in
...
| [ _; "serve" ] ->
  ...
  let on_exn exn =
    ...
    Lwt.async (fun () ->
        Lwt_io.eprintlf "❌ Unexpected exception during HTTP lifecycle: %s"
          (Libutils.Exception.human exn))
  in
    ...
    try%lwt
      let kind =
        ...
        | _ ->
          raise
            (Http_not_found
               (sprintf "Invalid endpoint or method: %s '%s'" (Code.string_of_method meth) path))
      in
      begin
        match meth, Header.get (Request.headers req) "api-key" with
        ...
        | meth, _ ->
          raise
            (Http_bad_request
               (sprintf "HTTP method must be POST, received: %s" (Code.string_of_method meth)))
      end;

      (* Serve request *)
      let parse_request p raw =
        match Yojson.Safe.from_string raw |> p with
        | Ok x -> x
      ...
      reply t0 200 req ~body ~message:(sprintf "OK %s" path) ()
    with
  ...
  Server.create ~timeout ~ctx:(Net.init ~ctx ()) ~mode:(`TCP (`Port port)) ~on_exn server
| argv ->
...
let () =
  Lwt_main.run
    (try%lwt main (Sys.get_argv () |> Array.to_list) with
    | (Failure _ as ex)
