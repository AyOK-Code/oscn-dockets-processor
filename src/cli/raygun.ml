...
let serialize ~tags ~custom ~machine_name ?req ?res_status exn =
  ...
  let class_name =
    match Sexp.of_string raw_message with
    | Sexp.List (Sexp.Atom name :: _) -> name
    | _
     |(exception _) ->
      raw_message
  in
  let stacktrace : Yojson.Basic.t list =
    List.map raw_stacktrace ~f:(fun line ->
        let default =
          `Assoc
            [
              "lineNumber", `Int 0;
        ...
        in
        match Re2.get_matches stacktrace_regex line with
        | Ok []
         |Error _ ->
          default
        | Ok (first :: _) -> (
          match Re2.Match.get_all (Re2.without_trailing_none first) with
              ...
              ( "tags",
                `List
                  (List.concat_no_order
                     [ List.map tags ~f:(fun x -> `String x); Lazy.force raygun_additional_tags ]) );
              "userCustomData", custom;
              "request", request;
              ( "response",
                Option.value_map res_status ~default:(`Assoc []) ~f:(fun status ->
...
let send ?(tags = []) ?(custom = `Assoc []) ?req ?res_status exn =
  match Lazy.force raygun_api_key with
  ...
  | Some raygun_api_key -> (
    ...
    | status ->
      failwithf "❗ Failed to save exception to Raygun (HTTP %d):\n%s\n%s" status
        (Response.headers res |> Header.to_frames |> String.concat ~sep:"\n")
        raw ()
  )
