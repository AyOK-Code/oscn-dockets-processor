...
let find_section ~html ~section_classes ~el_type ?empty_type () =
  ...
  | Some h2 ->
    begin match Soup.next_element h2, empty_type with
    ...
    | (Some _ as next), _ ->
      raise (Not_present (
          sprintf "Invalid page structure, could not find '%s' data (was %s instead of '%s')"
            section_class (Option.value_map next ~default:"not found" ~f:(fun x -> sprintf "'%s'" (Soup.name x))) el_type
        )
      )
    end
  end
...
let rec process_counts_open acc el =
  ...
  begin match Option.bind p ~f:(fun x -> Soup.texts x |> List.last) with
  | None -> acc
  | Some s ->
...
let process_counts_closed div =
  let open Soup.Infix in
  let first, second = begin match div $$ "table" |> Soup.to_list with
  ...
  in
  (* First table *)
  let count_as_filed, date_of_offense, violation_of1 = begin match first $$ "tbody tr td" |> Soup.to_list with
  | [number; description] ->
    begin match Soup.texts number with
    ...
    let violation_of = description $? "a" |> Option.map ~f:(fun el -> Soup.texts el |> String.concat ~sep:" " |> Text.clean) in
  ...
  in
  (* Second table *)
  second $$ "tbody tr" |> Soup.to_list |> List.map ~f:(fun tr ->
    let disposition, count_as_disposed, violation_of2, party = begin match tr $$ "td" |> Soup.to_list with
    | [_td1; td2; td3] ->
      let party = td_party ~f:String.uppercase td2 in
      begin match Soup.trimmed_texts td3 with
      | td1::td2::rest ->
...
let process ~name_matcher uri raw =
  ...
  let title, case_number, date_filed, date_closed, judge =
    ...
    begin match table $$ "tbody tr td" |> Soup.to_list with
    | [left; right] ->
      ...
      let judge_or_appellate =
        begin match Array.find_map fragments ~f:(String.chop_prefix ~prefix:"Judge: ") with
        | Some s -> s
        | None -> (begin match Array.find_map fragments ~f:(String.chop_prefix ~prefix:"Appealed from: ") with
          | Some s -> s
  ...
  let parties =
    ...
    let groups = Soup.descendants p |> Soup.to_list |> List.group ~break:(fun _left right ->
        Option.value_map (Soup.element right) ~default:false ~f:(fun el -> String.((Soup.name el) = "br"))
      )
    in
    ...
    List.iter groups ~f:(fun group ->
      ...
      let person_href = List.fold group ~init:None ~f:(fun acc node ->
          begin match Soup.element node with
          | None ->
  ...
  let events =
    ...
    table $$ "tbody tr" |> Soup.to_list |> Array.of_list_map ~f:(fun row ->
      begin match row $$ "td" |> Soup.to_list with
      | [td1; td2; td3; _td4] ->
        let party = td_party ~f:String.uppercase td2 in
        let datetime =
          ...
          $? "font"
          >>= td_party
          >>| Oscn.parse_datetime ~section:"event"
        in
        let description =
    ...
    |> Array.filter ~f:(fun event ->
      ...
      match event with
      | { party = None; datetime = None; description; judge = None; } when Text.(=) description empty -> false
      | _ -> true
      )
  in
  ...
  let completed_counts = begin match (html $$ "div.CountsContainer" |> Soup.to_list), status with
  | [], Open -> [||]
  ...
  let dockets =
    ...
    table $$ "tbody tr" |> Soup.to_list |> Array.of_list_map ~f:(fun row ->
      ...
      | [td1; td2; td3; td4; td5; td6] ->
        {
          date = Soup.texts td1 |> String.concat ~sep:" " |> Text.clean |> Text.require |> Option.map ~f:(Oscn.parse_date ~section:"docket");
          code = Soup.texts td2 |> String.concat ~sep:" " |> Text.clean |> Text.require;
          description = Option.value_map (Soup.child_element td3) ~default:"" ~f:(fun x -> Soup.texts x |> String.concat ~sep:" ") |> Text.clean;
          links = td3 $$ "a[href]" |> Soup.to_list |> List.filter_map ~f:(fun link ->
          ...
          party = Soup.texts td5 |> String.concat ~sep:" " |> Text.clean |> Text.uppercase |> Text.require;
          amount = Soup.texts td6 |> String.concat ~sep:" " |> Text.clean |> Text.require |> Option.map ~f:(fun x -> Text.to_string x |> Money.amount);
        }
