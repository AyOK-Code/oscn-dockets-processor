...
let except keys json : Yojson.Safe.t =
  `Assoc
    ...
    |> List.filter ~f:(fun (k, _v) -> Array.mem keys k ~equal:String.equal |> not)
    )
...
| Full_name { first_name; last_name } ->
  `String (sprintf "%s, %s" (Text.to_string last_name) (Text.to_string first_name))
...
type open_case_count = { description: Text.t [@key "Description__c"] } [@@deriving to_yojson] [@@unboxed]
